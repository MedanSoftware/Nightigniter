...
class Curl {
	...
	**/
	function request($method, $url, $vars = array()) {
		...
		$this->request = curl_close($this->request);
	...
	* @access protected
	**/
	protected function set_request_options($url, $vars, $proxy=false) {
		...
		if(!empty($proxy))
		{
			if(filter_var($proxy,FILTER_VALIDATE_BOOLEAN))
			{

			}
			else
			{
				curl_setopt($ch, CURLOPT_PROXY,return_if_exists($proxy,'ip',null));

				//Set the port.
				curl_setopt($ch, CURLOPT_PROXYPORT,return_if_exists($proxy,'port',null));
 ...
 * @param  array  $data    get/post data
 * @param  array  $headers headers
 * @return object
 */
function curl($method='get', $host, $data=array(), $headers=array())
{
	$curl = new Curl;
	(!empty($headers))?$curl->headers = array_merge($curl->headers, $headers):FALSE;
	return $curl->{strtolower($method)}($host,$data);
}